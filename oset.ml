...
module Set =
  (* This code is copied from Objective Caml standard library *)
  struct
    ...
    let rec split ~cmp x = function
      ...
      | Node(l, v, r, _) ->
          ...
          if c = 0 then (l, Some v, r)
            ...
            let (lr, vr, rr) = split ~cmp x r in (join l v lr, vr, rr)
    ...
    let partition ~cmp p s =
      let rec part (t, f as accu) = function
        | Empty -> accu
        | Node(l, v, r, _) ->
            part (part (if p v then (add ~cmp v t, f)
                        else (t, add ~cmp v f)) l) r in
      part (Empty, Empty) s
...
class ['a] c ?compare:(cmp=Pervasives.compare) l = object (_ : 'b)
  val mutable set =
    List.fold_left l ~init:Empty ~f:(fun acc x -> add ~cmp x acc)
  method contents : 'a t = set
  method set (s : 'b) = set <- s#contents
  method clear = set <- empty
  method is_empty = is_empty set
  method mem x = mem ~cmp x set
  method add x = set <- add ~cmp x set
  method remove x = set <- remove ~cmp x set
  method union (s : 'b) = {< set = union ~cmp set s#contents >}
  method inter (s : 'b) = {< set = inter ~cmp set s#contents >}
  method diff (s : 'b) = {< set = diff ~cmp set s#contents >}
  method compare (s : 'b) = compare ~cmp set s#contents
  method equal (s : 'b) = equal ~cmp set s#contents
  method subset (s : 'b) = subset ~cmp set s#contents
  method iter ~(f : 'a -> unit) = iter f set
  method fold : 'c. f:('a -> 'c -> 'c) -> 'c -> 'c = fun ~f -> fold f set
  method for_all ~f = for_all f set
  method exists ~f = exists f set
  method filter ~f = set <- filter ~cmp f set
  method partition ~f =
    let s1, s2 = partition ~cmp f set in
    {< set = s1 >}, {< set = s2 >}
  method cardinal = cardinal set
  method elements = elements set
  method min_elt = min_elt set
  method max_elt = max_elt set
  method choose = choose set
end

class ['a] f ?compare:(cmp=Pervasives.compare) l = object (_ : 'b)
  val set =
    List.fold_left l ~init:Empty ~f:(fun acc x -> add ~cmp x acc)
  method contents : 'a t = set
  method is_empty = is_empty set
